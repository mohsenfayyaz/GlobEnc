...
def extract_attentions(model, encoder_func, dataset_len, device="cpu", delete_sep=False):
    ...
    for id in tqdm(range(dataset_len)):
        ...
        with torch.no_grad():
            logits, attentions, norms = model(**encoded, output_attentions=True, output_norms=True)
            # logits:     [1, 2],
...
def build_ratio_residual_attentions(raw_attentions_list, norms_list):
    ...
    for idx in tqdm(range(len(raw_attentions_list))):
        ...
        r_ratio_attentions["W-Res"].append(__build_ratio_residual_attention(raw_attention, norms_list[8][idx]))

        r_ratio_attentions["Uniform-Res"].append(__build_ratio_residual_attention(np.ones_like(raw_attention)/len(raw_attention), norms_list[8][idx]))

    return r_ratio_attentions


def __build_ratio_residual_attention(raw_attention, ratio_matrix):
    """
    ...
    for layer in range(raw_attention.shape[0]):
        result_attention[layer] = __add_residual(raw_attention[layer], ratio_matrix[layer])
    return result_attention


def __add_residual(att_mat, ratios):
    """
